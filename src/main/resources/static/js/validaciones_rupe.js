...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const letrasNombre = /^[\p{L}\p{M}\s'’-]+$/u;
    const colorPermitido = /^[\p{L}\p{M}\s'’_\/-]+$/u;
    const telefono10 = /^[0-9]{10}$/;
    ...
    function validarColor(elemento) {
        if (!elemento) {
            return true;
        }
        ...
        if (valor.length < 3) {
            setMsg(
                elemento,
                "El color del perro debe tener al menos 3 caracteres."
            );
            return false;
        }
        ...
        if (valor.length > 80) {
            setMsg(
                elemento,
                "El color del perro no puede exceder 80 caracteres."
            );
            return false;
        }

        if (!colorPermitido.test(valor)) {
            setMsg(
                elemento,
                "El color del perro acepta letras, espacios, acentos, ñ, "
                + "diéresis, guion (-), guion bajo (_), diagonal (/) "
                + "y apóstrofe."
            );
            return false;
        }
    ...
    if (formulario) {
        formulario.addEventListener(
            ...
            function (event) {
                ...
                if (!colorValido && !primerInvalido) {
                    primerInvalido = color;
                }

                valido = colorValido && valido;
