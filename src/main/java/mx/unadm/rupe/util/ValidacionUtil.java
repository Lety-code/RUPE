...
public final class ValidacionUtil {
     ...
     * Letras Unicode:
     * \p{L} = cualquier letra
     *          (incluye á, é, í, ó, ú, ñ, ü, etc.)
     * \p{M} = marcas combinadas de acentuación.
     */

    private static final Pattern TEXTO_NOMBRE =
            Pattern.compile(
                    "^[\\p{L}\\p{M}\\s'’\\-]{2,120}$"
            );
    ...
    private static final Pattern FECHA_HTML =
            Pattern.compile(
                    "^[0-9]{4}-[0-9]{2}-[0-9]{2}$"
            );
    ...
    public static void validarTextoObligatorio(
        ...
        if (!TEXTO_NOMBRE.matcher(limpio).matches()) {
            ...
            throw new IllegalArgumentException(
                    "El campo " + campo
                    + " acepta letras, espacios, acentos, "
                    + "ñ, diéresis, guion y apóstrofe."
            );
        }
    }
    ...
    public static void validarColor(
            String color) {
        ...
        if (!COLOR.matcher(limpio).matches()) {
            ...
            throw new IllegalArgumentException(
                    "El color del perro acepta letras, "
                    + "espacios, acentos, ñ, diéresis, "
                    + "guion (-), guion bajo (_), "
                    + "diagonal (/) y apóstrofe."
            );
        }
    }

    public static void validarSenas(
            String senas) {
         ...
         * signos comunes
         *
         * Se rechazan < y > para evitar etiquetas HTML.
         */
        if (limpio.contains("<")
                || limpio.contains(">")) {

            throw new IllegalArgumentException(
                    "Las señas particulares no permiten "
                    + "los signos < o >."
            );
        }
    ...
    public static LocalDate validarFechaExtravio(
        ...
        if (!FECHA_HTML.matcher(limpia).matches()) {
            ...
            throw new IllegalArgumentException(
                    ...
                    + "con año de 4 dígitos, "
                    + "por ejemplo 2026-08-04."
            );
        }
